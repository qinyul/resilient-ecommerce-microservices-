...
func (h *PaymentCompletedHandler) HandlePaymentCompleted(ctx context.Context, d amqp.Delivery) (error, bool) {
	...
	if err := json.Unmarshal(d.Body, &event); err != nil {
		slog.ErrorContext(ctx, "Fatal: Failed to unmarshal payment event (Dead Letter)", "error", err)
		return err, false // Fatal: Don't requeue malformed JSON
	}

	slog.InfoContext(ctx, "Received payment completed event", "order_id", event.OrderID)

	// Update order status to PAID
	err := h.orderService.UpdateStatus(ctx, event.OrderID, domain.StatusPaid)
	if err != nil {
		slog.ErrorContext(ctx, "Transient: Failed to update order status (Requeue)", "order_id", event.OrderID, "error", err)
		return err, true // Transient: Requeue on failure
	}

	slog.InfoContext(ctx, "Order status updated to PAID", "order_id", event.OrderID)
	return nil, false
}
